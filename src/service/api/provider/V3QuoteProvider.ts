...
import {type GasCallRequest, GasMultiCallContract, QuoterV2Contract} from "../../abi";
...
@CacheKey('V3QuoteProvider')
export class V3QuoteProvider implements QuoteProvider {
  ...
  async getRouteWithQuotes(routes: RouteWithoutQuote[], gasModel: GasModel, isExactIn: boolean): Promise<RouteWithQuote[]> {
    ...
    const quoterV2 = this.baseApi.connectInfo().create(QuoterV2Contract)
